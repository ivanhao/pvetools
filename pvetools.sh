...
"
    ...
    #"a" "无脑模式" \
if [ $L = "en" ];then
    OPTION=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "Config apt source:" 25 60 15 \
    ...
    OPTION=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "配置apt镜像源:" 25 60 15 \
    "b" "更换为国内源" \
    "c" "关闭企业更新源" \
    "d" "还原配置" \
    "q" "返回主菜单" \
...
#config vim
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "Config VIM:" 12 60 4 \
    "a" "Install vim & simply config display." \
    "b" "Install vim & config 'vim-for-server'." \
    "c" "Uninstall." \
    3>&1 1>&2 2>&3)
else
    x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "安装配置VIM！" 12 60 4 \
    "a" "安装VIM并简单配置，如配色行号等。" \
...
chApm(){
    clear
    apm=$(
    whiptail --title " PveTools   Version : 2.3.0 " --menu "Config hard disks APM & AAM:
配置硬盘静音、降温：
    " 25 60 15 \
...
if [ $L = "en" ];then
    OPTION=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "Config hard disks spindown:" 25 60 15 \
    ...
    OPTION=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "配置硬盘自动休眠" 25 60 15 \
    "a" "配置硬盘自动休眠" \
    "b" "还原硬盘自动休眠配置" \
...
unsetVmN(){
    ...
    vmid=$(whiptail  --title " PveTools   Version : 2.3.0 " --menu "
...
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "Config Nested:" 25 60 15 \
    "a" "Enable nested" \
    "b" "Set vm to nested" \
    "c" "Unset vm nested" \
    "d" "Disable nested" \
    ...
    x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "配置嵌套虚拟化:" 25 60 15 \
    "a" "开启嵌套虚拟化" \
    "b" "开启某个虚拟机的嵌套虚拟化" \
    "c" "关闭某个虚拟机的嵌套虚拟化" \
    "d" "关闭嵌套虚拟化" \
...
clear
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "Config lm-sensors & proxmox ve display:" 25 60 15 \
    "a" "Install." \
    "b" "Uninstall." \
    3>&1 1>&2 2>&3)
else
    x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "配置Sensors:" 25 60 15 \
    "a" "安装配置温度显示" \
    "b" "删除配置" \
    3>&1 1>&2 2>&3)
fi
...
$(echo $cards) \
    ...
    if [ $exitstatus = 0 ];then
        ...
        if [ $DISTROS ];then
            ...
            list1=`echo $list|awk 'NR>1{print $1}'`
            vmid=$(whiptail  --title " PveTools   Version : 2.3.0 " --radiolist "
...
您已经配置过这个了!
                    ...
                    opt=$(whiptail  --title " PveTools   Version : 2.3.0 " --checklist "
Choose options:
选择选项：" 20 60 10 \
...
configVideo(){
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "Config PCI Video card Passthrough:" 25 60 15 \
    "a" "Config Video Card Passthrough" \
    "b" "Config Video Card Passthrough to vm" \
    3>&1 1>&2 2>&3)
else
    x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "配置PCI显卡直通:" 25 60 15 \
    "a" "配置物理机显卡直通支持。" \
    "b" "配置显卡直通给虚拟机。" \
    3>&1 1>&2 2>&3)
fi
...
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "Config PCI Passthrough:" 25 60 15 \
    "a" "Config IOMMU on." \
    "b" "Config IOMMU off." \
    ...
    x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "配置硬件直通:" 25 60 15 \
    "a" "配置开启物理机硬件直通支持。" \
    "b" "配置关闭物理机硬件直通支持。" \
    "c" "配置显卡直通。" \
    "d" "配置qm set 硬盘给虚拟机。" \
...
安装配置完成！" 10 60
    ...
    enterChroot(){
        ...
        if [ $L = "en" ];then
            x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "Enter chroot:" 25 60 15 \
            $(echo $c) \
            3>&1 1>&2 2>&3)
        else
            x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "进入chroot环境:" 25 60 15 \
            $(echo $c) \
            3>&1 1>&2 2>&3)
        fi
        exitstatus=$?
...
chroot后台运行环境已经运行，需要重启吗？
    ...
    configChroot(){
        if [ $L = "en" ];then
            x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "Config chroot & docker etc:" 25 60 15 \
            ...
            x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "配置chroot环境和docker等:" 25 60 15 \
...
输入格式错误，请重新输入：
                ...
                if [ $1 = 'add' ];then
                    ...
                    d=$(whiptail --title " PveTools Version : 2.3.0 " --checklist "
disk list:
已添加的硬盘:
...
选择硬盘：" 30 90 10 \
                    ...
                    3>&1 1>&2 2>&3)
                    exitstatus=$?
                    t=$(whiptail --title " PveTools Version : 2.3.0 " --menu "
Choose disk type:
选择硬盘接口类型：" 20 60 10 \
                    "sata" "vm sata type" \
                    "scsi" "vm scsi type" \
...
配置完成" 10 60
    ...
    if [ $L = "en" ];then
        x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "Config qm set disks:" 25 60 15 \
        "a" "set disk to vm." \
        "b" "unset disk to vm." \
        3>&1 1>&2 2>&3)
    else
        x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "配置qm set 物理硬盘给虚拟机:" 25 60 15 \
        "a" "添加硬盘给虚拟机。" \
        "b" "删除虚拟机里的硬盘。" \
        3>&1 1>&2 2>&3)
    fi
...
$maps
    ...
    setDns(){
        ...
        if [ `cat /etc/resolv.conf|grep 'nameserver'|wc -l` != 0 ];then
            if [ $L = "en" ];then
                d=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "DNS - Many Tools:
Detect exist nameserver,Please choose:
                ...
                d=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "DNS - 常用的工具:
检测到已经配置有dns服务器: \
...
请选择以下操作：
        ...
        if [ $L = "en" ];then
            x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "DNS - Many Tools:" 25 60 15 \
            "a" "8.8.8.8(google)." \
            "b" "223.5.5.5(alidns)." \
            3>&1 1>&2 2>&3)
        else
            x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "DNS - 常用的工具:" 25 60 15 \
            "a" "8.8.8.8(谷歌)." \
            "b" "223.5.5.5(阿里)." \
            3>&1 1>&2 2>&3)
        fi
...
释放内存？" 10 60 );then
    ...
    darkMode(){
        if [ $L = "en" ];then
            d=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "DarkMode - Many Tools:
            " 25 60 15 \
            "a" "Install." \
            "b" "Uninstall." \
            3>&1 1>&2 2>&3)
...
#----------------- 请选择以下操作：----------------- \
            d=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "界面黑暗模式 - 常用的工具:
            " 25 60 15 \
            "a" "安装." \
            "b" "卸载." \
            3>&1 1>&2 2>&3)
...
提取显卡vbios失败。" 10 60
    ...
    folder2ram(){
        if [ $L = "en" ];then
            x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "folder2ram:" 25 60 15 \
            "a" "install" \
            "b" "Uninstall" \
            3>&1 1>&2 2>&3)
        else
            x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "USB设备做为系统盘的优化:" 25 60 15 \
            "a" "安装。" \
            "b" "卸载。" \
            3>&1 1>&2 2>&3)
        fi
...
配置完成" 10 60
    ...
    autoResize(){
        if [ $L = "en" ];then
            d=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "autoResize ROOT partition - Many Tools:
            " 25 60 15 \
            "a" "start." \
            3>&1 1>&2 2>&3)
        else
#----------------- 请选择以下操作：----------------- \
            d=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "自动扩展ROOT分区可用空间 - 常用的工具:
            " 25 60 15 \
            "a" "运行." \
            3>&1 1>&2 2>&3)
        fi
    ...
    if [ $L = "en" ];then
        x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "Many Tools:" 25 60 15 \
        ...
        x=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "常用的工具:" 25 60 15 \
        "a" "局域网扫描。" \
        "b" "配置DNS。" \
        "c" "释放内存。" \
        "d" "speedtest测速" \
...
clear
if [ $L = "en" ];then
    OPTION=$(whiptail --title " PveTools   Version : 2.3.0 " --menu "
Github: https://github.com/ivanhao/pvetools

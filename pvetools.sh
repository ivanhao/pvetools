...
if [ $L = "en" ];then
    OPTION=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "Config apt source:" 25 60 15 \
    "a" "Automation mode." \
    ...
    OPTION=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "配置apt镜像源:" 25 60 15 \
    "a" "无脑模式" \
    "b" "更换为国内ustc.edu.cn源" \
    "c" "关闭企业更新源" \
    "d" "还原配置" \
...
clear
if [ $L = "en" ];then
    OPTION=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "Config samba:" 25 60 15 \
    ...
    OPTION=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "配置samba:" 25 60 15 \
...
#config vim
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "Config VIM:" 12 60 4 \
    "a" "Install vim & simply config display." \
    "b" "Install vim & config 'vim-for-server'." \
    "c" "Uninstall." \
    3>&1 1>&2 2>&3)
else
    x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "安装配置VIM！" 12 60 4 \
    "a" "安装VIM并简单配置，如配色行号等。" \
...
#-------------chCpu--main---------------
if [ $L = "en" ];then
    OPTION=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "Config Cpufrequtils:" 25 60 15 \
    "a" "Config cpufrequtils to save power." \
    "b" "Remove config." \
    3>&1 1>&2 2>&3)
else
    OPTION=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "安装配置CPU省电" 25 60 15 \
    "a" "安装配置CPU省电" \
    "b" "还原配置" \
    3>&1 1>&2 2>&3)
fi
...
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "Config Nested:" 25 60 15 \
    "a" "Enable nested" \
    "b" "Set vm to nested" \
    "c" "Unset vm nested" \
    "d" "Disable nested" \
    ...
    x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "配置嵌套虚拟化:" 25 60 15 \
    "a" "开启嵌套虚拟化" \
    "b" "开启某个虚拟机的嵌套虚拟化" \
    "c" "关闭某个虚拟机的嵌套虚拟化" \
    "d" "关闭嵌套虚拟化" \
...
clear
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "Config lm-sensors & proxmox ve display:" 25 60 15 \
    "a" "Install." \
    "b" "Uninstall." \
    3>&1 1>&2 2>&3)
else
    x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "配置samba:" 25 60 15 \
    "a" "安装配置温度显示" \
    "b" "删除配置" \
    3>&1 1>&2 2>&3)
fi
...
您已经配置过这个了!
                    ...
                    opt=$(whiptail  --title " PveTools   Version : 2.0.3 " --checklist "
Choose options:
选择选项：" 20 60 10 \
...
configVideo(){
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "Config PCI Video card Passthrough:" 25 60 15 \
    "a" "Config Video Card Passthrough" \
    "b" "Config Video Card Passthrough to vm" \
    3>&1 1>&2 2>&3)
else
    x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "配置PCI显卡直通:" 25 60 15 \
    "a" "配置物理机显卡直通支持。" \
    "b" "配置显卡直通给虚拟机。" \
    3>&1 1>&2 2>&3)
fi
...
if [ $L = "en" ];then
    x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "Config PCI Passthrough:" 25 60 15 \
    "a" "Config IOMMU on." \
    "b" "Config IOMMU off." \
    ...
    x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "配置硬件直通:" 25 60 15 \
    "a" "配置开启物理机硬件直通支持。" \
    "b" "配置关闭物理机硬件直通支持。" \
    "c" "配置显卡直通。" \
    "d" "配置qm set 硬盘给虚拟机。" \
...
安装配置完成！" 10 60
    ...
    enterChroot(){
        ...
        if [ $L = "en" ];then
            x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "Enter chroot:" 25 60 15 \
            $(echo $c) \
            3>&1 1>&2 2>&3)
        else
            x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "进入chroot环境:" 25 60 15 \
            $(echo $c) \
            3>&1 1>&2 2>&3)
        fi
        exitstatus=$?
...
输入格式错误，请重新输入：
                ...
                if [ $1 = 'add' ];then
                    ...
                    disks=`ls -alh /dev/disk/by-id|sed '/\.$/d'|sed '/^$/d'|awk 'NR>1{print $9" "$11" OFF"}'|sed 's/\.\.\///g'|sed '/wwn/d'|sed '/^dm/d'|sed '/lvm/d'`
...
配置完成" 10 60
    ...
    if [ $L = "en" ];then
        x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "Config qm set disks:" 25 60 15 \
        "a" "set disk to vm." \
        "b" "unset disk to vm." \
        3>&1 1>&2 2>&3)
    else
        x=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "配置qm set 物理硬盘给虚拟机:" 25 60 15 \
        "a" "添加硬盘给虚拟机。" \
        "b" "删除虚拟机里的硬盘。" \
        3>&1 1>&2 2>&3)
    fi
...
clear
if [ $L = "en" ];then
    OPTION=$(whiptail --title " PveTools   Version : 2.0.3 " --menu "
Github: https://github.com/ivanhao/pvetools
